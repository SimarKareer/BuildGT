...
def preprocess_corner(x, y, w, h, image):
    ...
    '''
    qCard.warp = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # Grab corner of warped card image and do a 4x zoom
    '''Qcorner = image'''
    Qcorner = qCard.warp[0:h, 0:w]
    Qcorner_zoom = cv2.resize(Qcorner, (0,0), fx=1, fy=1)
    cv2.imshow('zoom', Qcorner_zoom)

    # Sample known white pixel intensity to determine good threshold level
    white_level = Qcorner_zoom[15,int((CORNER_WIDTH*4)/8)]
    thresh_level = white_level - CARD_THRESH - 20
    print(thresh_level)
    if (thresh_level <= 0):
        thresh_level = 1
    retval, query_thresh = cv2.threshold(Qcorner_zoom, thresh_level, 255, cv2. THRESH_BINARY_INV)
    cv2.imshow('thresh', query_thresh)
